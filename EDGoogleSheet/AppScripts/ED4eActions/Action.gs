 //var charProps = SpreadsheetApp.getActiveSpreadsheet().getRange("Properties!ae2:ah1500");
//var propertyResults = SpreadsheetApp.getActiveSpreadsheet().getRange("Action!b2:e100");
...
function EDAction(sheetLocations) {
    return {
      ...
      error : function(type, message) {
        this.failureLog.push([this.target,this.characteristic,type,message,""]);
      },
      ...
      commit : function() {
        if (this.success) {
          ...
          // add new logs
          this.updateLogs([...this.failureLog, ...this.messageLog]);
          ...
          // write any failures regardless of whether the action was a success or not
          this.updateLogs(this.failureLog);
        }
